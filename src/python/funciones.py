...
def programacion_dinamica(B:List[List[int]], k:int, m1:int, m2:int, grilla_x, grilla_y, valores_x, valores_y, minimo:List[List[int]], error_total, error_minimo, estado):
        error_segmento1 = 0
        if len(B) == k:
            if error_total < error_minimo and B[-1][0] == m1-1:
                error_minimo = error_total
            ...
            if len(B) == 0:
                for i in range(m2):
                    B.append([0, i])
                    minimo, error_minimo = programacion_dinamica(B, k, m1, m2, grilla_x, grilla_y, valores_x, valores_y, minimo, error_total, error_minimo)
                    B.pop()
            elif error_total < error_minimo:
                for l in range(int(B[-1][0]) + 1, m1):
                    for j in range(m2):
